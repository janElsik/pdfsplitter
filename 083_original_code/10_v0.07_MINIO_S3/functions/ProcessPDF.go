...
func ProcessPDF(w http.ResponseWriter, r *http.Request, directoryPath string, splitToXPages int, dirName string) (tempFile string) {
	...
	update := false
	onTop := true
	wm, _ := api.TextWatermark("Confidential", "font:Courier, points:45, col: 1 0 0, rot:45, sc:1 abs", onTop, update, pdfcpu.POINTS)
	err = api.AddWatermarksFile(TempFile.Name(), "", nil /*[]string{"odd"}*/, wm, nil)
	if err != nil {
		fmt.Println(err)
	}
